...
function send_package($mysqli) {
    ...
    if($result->num_rows > 0){
        ...
        $row = $result->fetch_assoc();
        $courier_ID = $row['e'];
        $query_insert_assign_to_courier = "INSERT INTO assigns(package_ID, courier_ID, is_delivered) VALUES(package_ID, courier_ID, is_delivered)  (LAST_INSERT_ID(),'$courier_ID','by_customer')";
        $mysqli->query($query_insert_assign_to_courier) or die('Error in query: ' . $mysqli->error);
    }
    #-----------------------------------
